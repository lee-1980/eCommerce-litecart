<?php
  ...
  function form_draw_currency_field($currency_code, $name, $value=true, $parameters='') {
    ...
    return '<div class="input-group">' . PHP_EOL
         . '  <input '. (!preg_match('#class="([^"]+)?"#', $parameters) ? 'class="form-control"' : '') .' type="number" step="any" name="'. htmlspecialchars($name) .'" value="'. (!empty($value) ? round($value, currency::$currencies[$currency_code]['decimals']+3) : '') .'" data-type="currency"'. (($parameters) ? ' '. $parameters : false) .' />' . PHP_EOL
  ...
  function form_draw_decimal_field($name, $value=true, $decimals=2, $min=null, $max=null, $parameters='') {
    if ($value === true) $value = form_reinsert_value($name); if($_REQUEST['doc']==='add_product'&&(float)$value < 1) $value =1;
  ...
  function form_draw_file_field($name, $parameters='') {
          ...
          return '<input '. (!preg_match('#class="([^"]+)?"#', $parameters) ? 'class="form-control"' : '') .' type="file" multiple="multiple" name="'. htmlspecialchars($name) .'"'. (($parameters) ? ' '.$parameters : false) .' />'; 
