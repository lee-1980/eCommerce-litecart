<?php
  ...
  class sm_zone_weight_express_malaysia_offer_parcel {
    public $id = __CLASS__;
    ...
    // Calculate cart weight
      ...
      foreach ($items as $item) {
       ...
       $total_weight += weight::convert($item['quantity'] * (!empty($oversize_parcel_weight) && $oversize_parcel_weight * $item['quantity'] > 3 ? $oversize_parcel_weight:$item['weight']), $item['weight_class'], $this->settings['weight_class']);
      }
      ...
      if (empty($options)) {
        if (!empty($this->settings['weight_rate_table_x'])) {
          $cost = self::calculate_cost($this->settings['weight_rate_table_x'], ($total_weight) + 0.3);
