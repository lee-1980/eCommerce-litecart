<?php
  ...
  class sm_zone_weight_express_malaysia_reseller_10 {
    public $id = __CLASS__;
    ...
    private function calculate_oversize_parcel_weight( $dim_x = 0, $dim_y = 0, $dim_z = 0, $dim_class = "cm") {
      if ((empty($dim_x) && $dim_x == 0) || (empty($dim_y) && $dim_y == 0) || (empty($dim_z) && $dim_z == 0)) return 0;
       ...
       $weight = (length::convert($dim_x, $dim_class, "cm") + 1 ) * (length::convert($dim_y, $dim_class, "cm") + 1 ) * (length::convert($dim_z, $dim_class, "cm") + 1 ) / 6000;
