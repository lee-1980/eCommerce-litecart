<?php
  ...
  class sm_zone_weight_malaysia_2_normal {
    public $id = __CLASS__;
    ...
    // Calculate cart weight
      ...
      for ($i=1; $i <= 3; $i++) {
        ...
        $cost = self::calculate_cost($this->settings['weight_rate_table_'.$i], ($total_weight) + 0.7);
      ...
      if (empty($options)) {
        if (!empty($this->settings['weight_rate_table_x'])) {
          $cost = self::calculate_cost($this->settings['weight_rate_table_x'], ($total_weight) + 0.7);
