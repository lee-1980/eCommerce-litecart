<div id="content" class="twelve-eighty">
    ...
    <?php echo functions::form_draw_form_begin('checkout_form', 'post', document::ilink('combined_new_order_process'), false, 'autocomplete="off"'); ?>
...
<script>
    ...
    function runQueue() {
        ...
        switch (task.component) {
            case 'cart':
                url = '<?php echo document::ilink('ajax/combined_checkout_cart'); ?>';
                break;
            case 'customer':
                url = '<?php echo document::ilink('ajax/combined_checkout_customer'); ?>';
                break;
            case 'shipping':
                url = '<?php echo document::ilink('ajax/combined_checkout_shipping'); ?>';
                break;
            case 'summary':
                url = '<?php echo document::ilink('ajax/combined_checkout_summary'); ?>';
                break;
            default:
    ...
    $('#box-checkout .cart.wrapper').on('click', 'button[name="remove_combined_item"]', function(e){
        e.preventDefault();
        var data = 'token=' + $(':input[name="token"]').val()
            + '&' + $(this).closest('td').find(':input').serialize()
            + '&remove_combined_item=' + $(this).val();
