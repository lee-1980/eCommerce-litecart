<?php
  ...
  if (!empty($_GET['listing_link_id'])) {
    $listing_link = new ent_listing_link($_GET['listing_link_id']);
  } else {
    $listing_link = new ent_listing_link();
  }

  if (empty($_POST)) {
    foreach ($listing_link->data as $key => $value) {
      $_POST[$key] = $value;
    }
  }

  breadcrumbs::add(language::translate('title_listing_links', 'Listing Links'), document::link(WS_DIR_ADMIN, array('doc' => 'listing_links'), array('app')));
  breadcrumbs::add(!empty($listing_link->data['id']) ? language::translate('title_edit_listing_link', 'Edit Listing Links') :  language::translate('title_add_new_listing_link', 'Add New Listing Link'));
  ...
  if (isset($_POST['save'])) {
    ...
    try {
      ...
      if (!empty($_POST['code']) && database::num_rows(database::query("select id from ". DB_TABLE_LISTING_LINKS ." where id != '". (isset($_GET['listing_link_id']) ? (int)$_GET['listing_link_id'] : 0) ."' and code = '". database::input($_POST['code']) ."' limit 1;"))) throw new Exception(language::translate('error_code_database_conflict', 'Another entry with the given code already exists in the database'));
      ...
      foreach ($fields as $field) {
        if (isset($_POST[$field])) $listing_link->data[$field] = $_POST[$field];
      }

      $listing_link->save();

      if (!empty($_POST['delete_image'])) $listing_link->delete_image();

      if (is_uploaded_file($_FILES['image']['tmp_name'])) {
        $listing_link->save_image($_FILES['image']['tmp_name']);
      }

      notices::add('success', language::translate('success_changes_saved', 'Changes saved'));
      header('Location: '. document::link(WS_DIR_ADMIN, array('doc' => 'listing_links'), array('app')));
      exit;
  ...
  if (isset($_POST['delete'])) {
    ...
    try {
      if (empty($listing_link->data['id'])) throw new Exception(language::translate('error_must_provide_listing_link', 'You must provide a listing link'));
...
<div class="panel panel-app">
  <div class="panel-heading">
    <?php echo $app_icon; ?> <?php echo !empty($listing_link->data['id']) ? language::translate('title_edit_listing_link', 'Edit Listing Link') :  language::translate('title_add_new_listing_link', 'Add New Listing Link'); ?>
  </div>
  ...
  <div class="panel-body">
    <?php echo functions::form_draw_form_begin('listing_link_form', 'post', false, true); ?>
            ...
            <div class="col-md-6">
              <div id="image">
                <div class="thumbnail" style="margin-bottom: 15px;">
                  <img src="<?php echo document::href_link(WS_DIR_APP . functions::image_thumbnail(FS_DIR_APP . 'images/' . $listing_link->data['image'], 400, 100)); ?>" alt="" />
                </div>

                <div class="form-group">
                  <label><?php echo ((isset($listing_link->data['image']) && $listing_link->data['image'] != '') ? language::translate('title_new_image', 'New Image') : language::translate('title_image', 'Image')); ?></label>
                  <?php echo functions::form_draw_file_field('image', ''); ?>
                  <?php if (!empty($listing_link->data['image'])) { ?>
                  <div class="checkbox">
...
<script>
  ...
  $('input[name="image"]').change(function(e) {
    if ($(this).val() != '') {
      ...
      $('#image img').attr('src', '<?php echo document::href_link(WS_DIR_APP . functions::image_thumbnail(FS_DIR_APP . 'images/' . $listing_link->data['image'], 400, 100)); ?>');
    }
  });
